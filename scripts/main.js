            ...
            $.getJSON(jsontxt, function (data) {
                ...
                let becPromise = new Promise((resolve, reject) => {
                    if (bec.length == 1) {
                        ...
                        results = data.filter(function (x) { return x["BECvar_site"] == bec_name && x["HTp_pred"] >= suit});
                        output_suit = data.filter(function (x) { return x["BECvar_site"] == bec_name && x["HTp_pred"] >= suit});
